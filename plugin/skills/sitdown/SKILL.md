...
allowed-tools: Read Grep Glob Bash(git log *) Bash(git status *) Bash(mkdir -p .capo/hits) Bash(cat .capo/*)
effort: high
---
...
5. **Write the file** to `.capo/hits/<slug>.yaml` and print the path.
6. **Stop.** Show the list, then wait. Do not start work in the same turn.
